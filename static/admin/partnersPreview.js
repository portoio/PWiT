var PartnersPreview = createClass({
  render: function() {
    var entryTiers = this.props.entry.getIn(['data', 'partner_tier']);
    ...
    return h(
      ...
      h(
        "div",
        { className: "footer__partners-wrapper" },
        h(
          "div",
          { className: "footer__partners" },
          entryTiers.map((entryTier) => {
            var tierTitle = entryTier.get('title');
            var tierLogos = entryTier.get('logos');

            return h(
              "div",
              { className: "footer__partners-left" },
              h(
                "h1",
                { className: "footer__partners-title" },
                tierTitle
              ),
              tierLogos.map((tierLogo) => {
                var logoURL = tierLogo.getIn([ 'data', 'link' ]);
                var logoImage = tierLogo.get('image') ? getAsset(tierLogo.get('image')).toString() : '/admin/default.jpg'

                return h(
                  "a",
                  { className: "footer__partners__link", target: "_blank", href: logoURL },
                  h("img", { src: logoImage })
                )
              })
            );
